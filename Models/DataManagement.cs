...
namespace LaborNeedsScheduling.Models
{
    ...
    public class WorkWeek
    {
        ...
        /// </summary>
        public DateTime weekMarker = DateTime.Now.StartOfWeek(DayOfWeek.Sunday);
        //public DateTime currentWeekMarker = DateTime.Today.AddDays(-1 * (int)DayOfWeek.Saturday);
        ...
        //power hours moved from HourOfWorkDay class
        public double[] powerHours = new double[7];
        public double[] lowHours = new double[42];
        public string[] powerHourRows = new string[7];
        public string[] lowHourRows = new string[42];
        ...
        {
            DataTable FoundEmployees = FakeAPI.FindEmployee(EmployeeId);

            DataTable EmployeeInfo = new DataTable();

            if (FoundEmployees.Rows.Count == 0)
            {
                EmployeeInfo.Columns.Add("No employees found");
                return EmployeeInfo;
            }
            else
            {
            ...
            for (int i = 0; i < AllocatedHoursDisplay.Rows.Count; i++)
            {
                for (int n = 0; n < 8; n++)
                {
                    if (n > 0)
                    {
                        ...
                        if (Convert.ToString(AllocatedHoursDisplay.Rows[i][0]) == powerHourRows[n - 1])
                        {
                            if (dayCheck[n] == false)
                            {
            ...
            for (int i = 0; i < AllocatedHoursDisplay.Rows.Count; i++)
            {
                for (int n = 0; n < 8; n++)
                {
                    if (n > 0)
                    {
                        ...
                        {
                            if (n == columnCount[h] && Convert.ToString(AllocatedHoursDisplay.Rows[i][0]) == lowHourRows[h])
                            {
                                LowHourCells[y] = true;
        ...
        public void FillPowerHourForecastTable()
        {
            ...
            try
            {
                        ...
                        if (WeightedAverageTrafficTotal.Rows[row][col].ToString() != ""/* && WeightedAverageTrafficTotal.Rows[row][col].ToString() != "0"*/)
                        {
                            if (row <= PowerHourForecast.Rows.Count - 6)
                            {
                                ...
                                forecast = currentHour + Convert.ToDouble(WeightedAverageTrafficTotal.Rows[row + 1][col])
                                    + Convert.ToDouble(WeightedAverageTrafficTotal.Rows[row + 2][col])
                                    + Convert.ToDouble(WeightedAverageTrafficTotal.Rows[row + 3][col])
                                    + Convert.ToDouble(WeightedAverageTrafficTotal.Rows[row + 4][col])
                                    + Convert.ToDouble(WeightedAverageTrafficTotal.Rows[row + 5][col]);
                ...
                {
                    powerHours[i] = dayHighs[i];
                }

                // low hours for each day (excluding zeros)
                for (int i = 0; i < dayLows.Length; i++)
                {
                    lowHours[i] = dayLows[i];
                }

                // get the names of the rows containing power hours
                for (int i = 1; i <= powerHourRows.Length; i++)
                {
                    for (int j = PowerHourForecast.Rows.Count - 1; j >= 0; j--)
                    {
                        if (Convert.ToString(PowerHourForecast.Rows[j][i]) == Convert.ToString(powerHours[i - 1]))
                        {
                            //HourOfWorkDay.powerHourRows[i - 1] = Convert.ToString(j);
                            powerHourRows[i - 1] = Convert.ToString(PowerHourForecast.Rows[j][0]);
                            break;
                        }
                    }
                }
                ...
                for (int i = 1; i <= powerHourRows.Length; i++)
                {
                    ...
                    {
                        int count = j / 2;
                        if (Convert.ToString(PowerHourForecast.Rows[j][i]) == Convert.ToString(lowHours[lowHourIterator])
                            || Convert.ToString(PowerHourForecast.Rows[j][i]) == Convert.ToString(lowHours[lowHourIterator + 1])
                            || Convert.ToString(PowerHourForecast.Rows[j][i]) == Convert.ToString(lowHours[lowHourIterator + 2])
                            || Convert.ToString(PowerHourForecast.Rows[j][i]) == Convert.ToString(lowHours[lowHourIterator + 3])
                            || Convert.ToString(PowerHourForecast.Rows[j][i]) == Convert.ToString(lowHours[lowHourIterator + 4])
                            || Convert.ToString(PowerHourForecast.Rows[j][i]) == Convert.ToString(lowHours[lowHourIterator + 5]))
                        {
                            lowHourRows[lowCount] = Convert.ToString(PowerHourForecast.Rows[j][0]);
                            counter++;
                            lowCount++;
                        }
                        if (counter == 6)
        ...
        public void RedistributeHours()
        {
            ...
            for (int i = 1; i < 8; i++)
            {
                ...
                string powerHour = powerHourRows[i - 1];
        ...
        public List<DateTime> GetEveryThirdSunday()
        {
